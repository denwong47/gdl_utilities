...
if (archicad):
	...
	class connection():
		...
		def wait_for_alive(
			...
			while (
					(
						(max_wait is None) or (max_wait < 0) or \
						(timer.perf_counter() - _start) <= max_wait 
					) and \
				not(self.alive)
			):
				self.__init__()
	...
	connector = ACConnectionModuleNotFound(
		"archicad Module cannot be imported - check that it is installed in the current environment through 'python3 -m pip instal archicad'."
	)